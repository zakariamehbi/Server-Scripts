...
$IPTABLES -A INPUT -p tcp -m multiport --dports $HTTP -m state --state NEW -m recent --set
$IPTABLES -A INPUT -p tcp -m multiport --dports $HTTP -m state --state NEW -m recent --update --seconds $SECONDS --hitcount $CONNECTIONS -j DROP
