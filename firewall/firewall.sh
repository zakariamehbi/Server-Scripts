...
case "$1" in
    start)
        ...
        if [ -f ./firewall.conf ] ; then
            ...
            echo "no configuration-file found in ./firewall.conf"
        ...
        ## ACCEPT ALL CONNECTIONS FROM IPV4 WHITELIST FILE ./ip-whitelist-ipv4.conf
        for IP in `cat ./ip-whitelist-ipv4.conf | sed 's/\s/_/g' | grep -v "^#"`; do
            $IPTABLES -A INPUT -s $IP -m state --state NEW -j ACCEPT
        done;

        ## ACCEPT ALL CONNECTIONS FROM IPV6 WHITELIST FILE ./ip-whitelist-ipv6.conf
        for IP in `cat ./ip-whitelist-ipv6.conf | sed 's/\s/_/g' | grep -v "^#"`; do
            $IP6TABLES -A INPUT -s $IP -m state --state NEW -j ACCEPT
        done;

        ## DROP ALL CONNECTIONS FROM BLACKLIST FILE ./ip-blacklist.conf
        for IP in `cat ./ip-blacklist.conf | sed 's/\s/_/g' | grep -v "^#"`; do
            $IPTABLES -A INPUT -s $IP -m state --state NEW -j DROP
        done;

        # EXECUTE ALL CUSTOM SCRIPTS IN ./custom
        if [ "$(ls -A ./custom)" ]; then
            for F in ./custom/*; do
                . $F
                echo "Execute: $F"
        ...
        # ALLOWED PORTS/PROTOCOLS FROM ./services.conf
        cat ./services.conf | grep -v "^#" | while read line; do
            [ -z "$line" ] && continue
